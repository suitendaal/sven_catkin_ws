...
VIVEPoseNode::VIVEPoseNode(int rate)
  : loop_rate_(rate)
  , nh_()
  , tf_broadcaster_()
  , tf_listener_()
  ...
  , world_yaw_(0)
{
  ...
  sub_L = it.subscribe("/image_left", 1, &VIVEPoseNode::imageCb_L, this);
  sub_R = it.subscribe("/image_right", 1, &VIVEPoseNode::imageCb_R, this);
  sub_i_L = nh_.subscribe("/camera_info_left", 1, &VIVEPoseNode::infoCb_L, this);
  sub_i_R = nh_.subscribe("/camera_info_right", 1, &VIVEPoseNode::infoCb_R, this);
...
VIVEPoseNode::~VIVEPoseNode()
{
  return;
}

bool VIVEPoseNode::Init()
{
...
void VIVEPoseNode::Shutdown()
{
  vr_.Shutdown();
}

bool VIVEPoseNode::setOriginCB(std_srvs::Empty::Request& req, std_srvs::Empty::Response& res)
{
...
void VIVEPoseNode::set_feedback(sensor_msgs::JoyFeedbackConstPtr msg) {
...
#ifdef USE_IMAGE
void VIVEPoseNode::imageCb_L(const sensor_msgs::ImageConstPtr& msg){
...
void VIVEPoseNode::imageCb_R(const sensor_msgs::ImageConstPtr& msg){
  if(msg->width > 0 && msg->height > 0 ){
    try {
...
void VIVEPoseNode::infoCb_L(const sensor_msgs::CameraInfoConstPtr& msg){
...
void VIVEPoseNode::infoCb_R(const sensor_msgs::CameraInfoConstPtr& msg){
  if(msg->K[0] > 0.0 && msg->K[4] > 0.0 ){
