...
import numpy as np
from .multi_least_squares_filter import *
from sven_ros.jump_detection.datalib import *

class MultiForceDerivativePredictor(MultiLeastSquaresFilter):
	"""docstring for Filter."""
	
	def __init__(self, **kwargs):
		super(MultiForceDerivativePredictor, self).__init__(**kwargs)
		self.time_window = kwargs.get('time_window',0.05)
		self.order = 0
		self.last_jump_time = None
		
	def copy(self):
		result = MultiForceDerivativePredictor(window_length=self.window_length, time_window=self.time_window)
