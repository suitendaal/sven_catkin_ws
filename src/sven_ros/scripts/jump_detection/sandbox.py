...
for phase in range(z.n_phases):
	z_pos.append(z.demo_variables[0].get_extended_derivative(phase))
...
for i in range(len(z_pos)):
	...
	plt.plot(phase_data.time, phase_data.value,'C' + str(i) + '-*',linewidth=config.linewidth, markersize=config.markersize2)
